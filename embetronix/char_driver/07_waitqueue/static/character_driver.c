...
#include<linux/kthread.h> /* for kernel thread creation */
...
DECLARE_WAIT_QUEUE_HEAD(wait_queue_chl);
/************************/
/***********************************************************/
...
/**********wait queue related function declarations*********/
...
static ssize_t chl_read(struct file *file_ptr,char __user *buffer,size_t length,loff_t *offset)
{
	...
	/* wakup waiting queue thread*/
	wait_queue_flag = 1;
	wake_up_interruptible(&wait_queue_chl);
...
{
	while(1){
		printk(KERN_INFO "CHL Driver's wait_function(): waiting for event..\n");
		wait_event_interruptible(wait_queue_chl,(wait_queue_flag!=0));
		if(wait_queue_flag == 2){
			printk(KERN_INFO "CHL Driver's wait_function(): Event came from exit function\n");
			return 0;
		}
		printk(KERN_INFO "CHL Driver's wait_function(): Event came from read function. Read count = %d\n",++read_count);
		wait_queue_flag=0;
	}
	do_exit(0);
	return 0;
...
static int __init chl_init(void)
{
	...
	init_waitqueue_head(&wait_queue_chl);

	/* Create kernel thread */
	wait_thread = kthread_create(wait_function,NULL,"waitthread");
	if(!wait_thread){
		printk(KERN_INFO "CHL Driver's init() : Kernel thread creation failed\n");
		goto DEVICE_ADD_ERROR;
	}
